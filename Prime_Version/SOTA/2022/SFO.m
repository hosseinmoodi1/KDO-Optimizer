...
%
% Description:
%   A bio-inspired metaheuristic based on the adaptive flight dynamics of
%   swift birds. Three complementary flight modes are modelled:
%     - Glide mode  : long-range exploration via Gaussian + Lévy noise
%     - Target mode : directed exploitation toward pbest and gbest
%     - Micro mode  : fine-grained refinement around gbest
%   A stagnation-aware reinitialization mechanism restores diversity.
%
% Inputs:
%   nPop     - Population size
%   MaxIter  - Maximum number of iterations
%   lb, ub   - Lower/Upper bounds (scalar or 1 x dim vector)
...
%   BestPos          - Best solution vector found
%   ConvergenceCurve - Best-so-far fitness at each iteration
% -------------------------------------------------------------------------

    lb = lb(:)';  ub = ub(:)';
    FEs = 0;

    % ---- Algorithm parameters (Table 6 of the paper) ----
    w         = 0.5;     % Inertia weight
    alpha_g   = 0.5;     % Gaussian scaling factor
    beta_l    = 0.57;    % Lévy scaling factor
    psi1      = 1.45;    % Cognitive acceleration coefficient
    psi2      = 1.45;    % Social acceleration coefficient
    StagLimit = 20;      % Stagnation tolerance for reinitialization
    ...
    for t = 1:MaxIter
        ...
        for i = 1:nPop
            ...
            % ----- Mode selection based on exploration/exploitation balance -----
            r_mode = rand();
            if r_mode < (1 - progress)
                mode = 1;   % Glide  (exploration)
            elseif r_mode < (1 - progress) + 0.6 * progress
                mode = 2;   % Target (exploitation)
            else
                mode = 3;   % Micro  (deep exploitation)
            end

            switch mode
                case 1  % Glide Mode (Eq. 1b in the paper)
                    L = levy_step(dim, 1.5);
                    V(i,:) = w * V(i,:) + alpha_g * randn(1, dim) + beta_l * L;
                    Xnew   = X(i,:) + V(i,:);

                case 2  % Target Mode (Eq. 4a)
