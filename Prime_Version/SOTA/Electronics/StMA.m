...
% -------------------------------------------------------------------------
    ...
    for t = 1:MaxIter
        ...
        ratio_sig = 1 - sigma_t / (sigma0 + eps);
        ratio_sig = min(max(ratio_sig, 0), 1);          % clamp to [0,1]
        eta_t = eta_min_p + (eta_max_p - eta_min_p) * ratio_sig;
