function [Best_score, Best_pos, cg_curve] = RUN(PopSize, MaxIter, lb, ub, dim, fobj, MaxFEs)
    % RUN: Runge Kutta Optimizer (Corrected to match original paper)
    ...
    % Algorithmic parameters (Eq. 18-21)
    a = 20;  
    b = 12;  
    c = 2;
    ...
    while FEs < MaxFEs
        % Adaptive scaling factor (Eq. 14)
        f = 0.2 + 0.3 * rand();
        
        % Sort population to find best solution
